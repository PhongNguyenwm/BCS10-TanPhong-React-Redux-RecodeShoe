...
export let shoeReducer = (state = intialState, action) => {
  switch (action.type) {
    //chức năng view sp
    case VIEW_DETAIL: {
    ...
    //chức năng xoá sp
    case DELETE_SHOE: {
      //từ id tìm index từ index dùng splice/filter để xoá shoe
      let newCart = state.cart.filter((item) => item.filter !== action.payload);
    ...
    case DECREASE: {
      ...
      let index = cloneCart.findIndex((item) => item.id === action.payload);
      if (index !== -1) {
        //giảm sl sp khi click button -
        if (cloneCart[index].amount > 0) {
          cloneCart[index].amount--;
        }

        //xoá sp khỏi cart khi về 0
        if (cloneCart[index].amount === 0) {
